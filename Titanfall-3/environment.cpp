...
void Player::SetAlive() { alive = true; }
...
pair<int&, int&> Player::GetCords() { return pair<int&, int&>{x, y}; }
...
vector<pair<int, int>> Func_trace (string& func_enter, pair<int, int>& cords, vector<Obstacle>& obstacle_mini,
    ...
    for (int x = 0; x <= size_map_x; ++x)  // перебираем х
    {
        ...
        else
        {
            ...
            if (not(isnan(ans)))  // значение non
            {
                ...
                auto obst_for =
                    find(black_cords.begin(), black_cords.end(), p);  // для проверки на попадание в препятсвие
