...
SOCKET Socket (addrinfo* result)
{
    ...
    {  // не получилось запустить
        perror(("socket failed #" + std::to_string(WSAGetLastError())).c_str());
...
void Bind (SOCKET sock, addrinfo* result)
{
    ...
    if (res == SOCKET_ERROR)
    {
        perror(("bind failed #" + std::to_string(WSAGetLastError())).c_str());
        freeaddrinfo(result);
        closesocket(sock);
        WSACleanup();
        exit(EXIT_FAILURE);
...
void Listen (SOCKET sock, int backlog)
{
    ...
    if (res == SOCKET_ERROR)
    {
        perror(("listen failed #" + std::to_string(WSAGetLastError())).c_str());
        closesocket(sock);
        WSACleanup();
        exit(EXIT_FAILURE);
    }
...
SOCKET Accept (SOCKET sock, struct sockaddr* addr, socklen_t* addr_len)
{
    ...
    if (res == INVALID_SOCKET)
    {
        perror(("accept failed #" + std::to_string(WSAGetLastError())).c_str());
        closesocket(sock);
        WSACleanup();
        exit(EXIT_FAILURE);
    }
...
void Connect (SOCKET sock, addrinfo* result)
{
    ...
    if (res == SOCKET_ERROR)
    {  // не получилось подключиться - удаляем сокет, потому что можем
...
void Inet_pton (int af, const char* src, void* dst)
{
    ...
    {  // все остальные ошибки
        perror(("inet_pton failed #" + std::to_string(WSAGetLastError())).c_str());
        WSACleanup();
        exit(EXIT_FAILURE);
    }
}
